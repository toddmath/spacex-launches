...
import { AsyncStorage, UsePersistedState, PersistedState } from './types'
...
export default function createAsyncPersistedState(
  ...
  const usePersistedState = <T>(
    ...
    const setPersistedState = async (
      ...
      const newItem = getNewItem<T>(
        key,
        persistedItem[safeStorageKey],
        newValue
      )
    ...
    useEffect(() => {
      const setInitialValue = async () => {
        ...
        const initialOrPersistedValue = getPersistedValue<T>(
          key,
          initialValue,
          persist[safeStorageKey]
        )
